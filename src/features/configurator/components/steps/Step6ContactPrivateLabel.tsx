...
export function Step6ContactPrivateLabel() {
  ...
  const handleFormSubmission = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log('📤 Invio form Private Label:', formData)
  ...
  const handleStripePayment = async (): Promise<boolean> => {
    ...
    try {
      ...
      const orderData: OrderData = {
        ...
        currency: 'eur',
        description: 'Campione Private Label',
        metadata: {
          serviceType: 'private-label',
          country: state.country,
          sessionId: state.sessionId,
          beverageType: state.beverageSelection?.selectedBeverage || 'unknown'
        }
      }

      console.log('💳 Avvio pagamento Stripe Private Label:', orderData)
    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-gray-200 pb-6">
        ...
        <p className="text-gray-600 text-sm">
          Compila i tuoi dati per ricevere il preventivo Private Label personalizzato
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="border-t border-gray-200 pt-6">
          ...
          <p className="text-center text-xs text-gray-500 mt-3">
            {state.wantsSample ? 
              'Procederai al pagamento sicuro per il campione Private Label' :
              'Riceverai un preventivo dettagliato via email entro 24 ore'
            }
          </p>
        </div>
      </form>
